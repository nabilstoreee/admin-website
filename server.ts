...
app.post("/api/auth/login", (req, res) => {
  ...
  if (user.isMuted) {
    return res.status(403).json({ success: false, message: "Email ini sudah dimute sama admin." });
  }
...
// Mute User (ONLY ADMIN)
app.post("/api/admin/users/mute", (req, res) => {
  const admin = getUserFromRequest(req);
  if (!admin || !admin.isAdmin) return res.status(403).json({ success: false, message: "Akses ditolak" });
  const { email, mute } = req.body;
  const db = getDb();
  const targetUser = db.users.find((u: any) => u.email.toLowerCase() === email.toLowerCase().trim());
  if (!targetUser) return res.status(404).json({ success: false, message: "User tidak ditemukan" });
  targetUser.isMuted = !!mute;
  saveDb(db);
  res.json({ success: true, message: `User ${email} ${mute ? 'dimute' : 'di-unmute'}!` });
});
...
app.post("/api/admin/users/delete", (req, res) => {
  ...
  if (!admin || !admin.isAdmin) {
    console.log("Delete attempt failed: Not an admin or no user found");
    return res.status(403).json({ success: false, message: "Akses ditolak" });
  }
  const { email } = req.body;
  const db = getDb();
  console.log("Database user list length:", db.users.length);
  const index = db.users.findIndex((u: any) => u.email.toLowerCase() === email.toLowerCase().trim());
  if (index === -1) {
    console.log("Delete attempt failed: User not found", email);
    return res.status(404).json({ success: false, message: "User tidak ditemukan" });
  }
